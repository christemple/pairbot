...
PairBot.prototype._setConfig = function (originalMessage) {
  ...
  this.usersToPair = this.users.filter(function(user) {
 ...
 * @private
 */
PairBot.prototype._welcomeMessage = function () {
    this.postMessageToChannel(
      this.channels[0].name, 'Hi! Just mention me `@' + this.name + '` and I\'ll pair everyone up in the channel :)',
      {as_user: true}
    );
};
